...
describe("applyFontSize", () => {
  ...
  it("mirrors the value to localStorage under 'dystil-font-size'", () => {
    applyFontSize("18px");
    expect(localStorage.getItem("dystil-font-size")).toBe("18px");
  });

  it("overwrites a stale localStorage value when the size changes", () => {
    applyFontSize("14px");
    expect(localStorage.getItem("dystil-font-size")).toBe("14px");
    applyFontSize("20px");
    expect(localStorage.getItem("dystil-font-size")).toBe("20px");
  });
});
...
describe("readSavedFontSize", () => {
  ...
  it("returns the saved value when it is a valid font size", () => {
    localStorage.setItem("dystil-font-size", "20px");
  ...
  it("returns the default when localStorage holds an unrecognised value", () => {
    ...
    localStorage.setItem("dystil-font-size", "99px");
    expect(readSavedFontSize()).toBe(FONT_SIZE_DEFAULT);
  });
