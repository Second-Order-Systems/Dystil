...
export interface WatchAuthorizationStatusOptions {
  ...
  /**
   * Continue polling even when `document.hidden` is true (e.g. the dystil
...
// ---------------------------------------------------------------------------
// Controller — Dystil-facing wrappers around the permission-flow plugin
// ---------------------------------------------------------------------------

/**
 * Dystil-facing alias: every OS permission we surface in onboarding,
 * recovery, or settings cards. Some entries (automation,
...
function isDystilAppBundle(
  appPath: string | null | undefined,
): appPath is string {
  if (!appPath || !appPath.endsWith(".app")) return false;
  const bundleName = appPath.split("/").at(-1) ?? "";
  // Covers Dystil.app plus the configured Development, beta, and enterprise
  // bundles, without enabling a drag flow for some arbitrary .app path.
  return /^Dystil(?:[ -].+)?\.app$/i.test(bundleName);
}
...
async function getDystilAppPath(): Promise<string | null> {
  if (state.appPath !== undefined) return state.appPath;
  const appPath = await suggestedHostAppPath();
  state.appPath = isDystilAppBundle(appPath) ? appPath : null;
...
async function restoreAlwaysOnTopWhenDystilRefocuses(): Promise<void> {
  ...
  try {
    ...
    const unlisten = await window.onFocusChanged(async ({ payload: focused }) => {
      ...
      // Hide the drag panel when dystil is frontmost — it should only
...
async function letSystemSettingsComeForward(): Promise<void> {
  try {
    ...
    await restoreAlwaysOnTopWhenDystilRefocuses();
  } catch {
...
async function reclaimDystilWindow(): Promise<void> {
  if (state.pendingFocusUnlisten) {
    try { state.pendingFocusUnlisten(); } catch {}
    state.pendingFocusUnlisten = null;
  }
  ...
  try {
    ...
    console.error(
      "failed to refocus dystil window after permission flow:",
      error,
    );
  }
}
 ...
 *
 * Calendar and automation are prompt/toggle based rather than
...
export async function requestPermissionWithFlow(
  ...
  // A previous unfinished controller can retain its old permission target.
  // Tear it down before beginning a different request.
  await stopActiveFlow();
  await letSystemSettingsComeForward();
  const dragPermission = dragPermissionByOSPermission[permission];
  const appPath = dragPermission ? await getDystilAppPath() : null;
