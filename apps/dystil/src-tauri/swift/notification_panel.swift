...
// Dystil notification panel styling.
...
struct NotificationContentView: View {
    ...
    var body: some View {
        ...
        // in non-activating panels. dystil:// URLs go through the
...
struct MarkdownText: View {
    ...
    /// `viewerOverridePath` is set when the link is a `dystil://view?path=…`
    ...
    /// If `url` is `dystil://view?path=…`, return the decoded path. Else nil.
    fileprivate static func viewerOverridePath(for url: URL) -> String? {
        guard url.scheme == "dystil" else { return nil }
                ...
                if let closeBracket = afterBracket.firstIndex(of: "]") {
                    ...
                    if afterClose.hasPrefix("(") {
                        ...
                        if let closeParen = urlPart.firstIndex(of: ")") {
                            ...
                            // `%2F`s in a `dystil://view?path=…` link
...
/// Tiny ↗ button rendered next to a `dystil://view?path=…` link so the
...
/// Open a URL with the right transport: in-app for dystil:// (no
...
public func notifShow(_ jsonPtr: UnsafePointer<CChar>) -> Int32 {
    ...
    guard let data = json.data(using: .utf8) else {
        NSLog("[dystil-notif] failed to convert json to utf8 data")
        return -1
    }
    let payload: NotificationPayload
    do {
        ...
        NSLog("[dystil-notif] json decode failed: \(error) — json: \(json.prefix(500))")
        return -1
    }
