...
const UI_NOTIFICATIONS_DISABLED: bool = true;
...
pub async fn send_notification(
    ...
    // Rewrite file-path markdown links to dystil://view?path=… so they
    ...
    if UI_NOTIFICATIONS_DISABLED {
        debug!("notify: UI notifications muted; persisted payload only");
        return Ok(Json(ApiResponse {
            success: true,
            message: "notification persisted without ui".to_string(),
        }));
    }
