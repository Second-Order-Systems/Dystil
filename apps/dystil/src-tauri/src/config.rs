...
use crate::dystil_paths::data_dir;
...
/// custom path was unusable and we silently fell back to default (~/.dystil or DYSTIL_DATA_DIR).
pub fn resolve_data_dir(data_dir_setting: &str) -> (PathBuf, bool) {
    let default_path = data_dir();

    // "default" or empty → use ~/.dystil
...
mod tests {
    ...
    fn test_resolve_valid_absolute_path() {
        let tmp = std::env::temp_dir().join("dystil_test_resolve");
