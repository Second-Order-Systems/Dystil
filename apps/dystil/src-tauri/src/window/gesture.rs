...
pub fn init_magnify_handler(app: tauri::AppHandle) {
    ...
    if Class::get("DystilMagnifyHandler").is_none() {
        let superclass = Class::get("NSObject").unwrap();
        let mut decl = ClassDecl::new("DystilMagnifyHandler", superclass).unwrap();
    ...
    if Class::get("DystilScrollInterceptor").is_none() {
        ...
        let superclass = Class::get("NSObject").unwrap();
        let decl = ClassDecl::new("DystilScrollInterceptor", superclass).unwrap();
        decl.register();
    }
}
...
pub(crate) unsafe fn attach_magnify_gesture_to_view(view: tauri_nspanel::cocoa::base::id) {
    with_autorelease_pool(|| {
        ...
        let handler_class = match Class::get("DystilMagnifyHandler") {
            Some(cls) => cls,
            None => {
                tracing::warn!(
                    "DystilMagnifyHandler class not registered yet — skipping gesture attach"
                );
                return;
            }
        };

        // Check if we already added our recognizer (look for DystilMagnifyHandler target)
