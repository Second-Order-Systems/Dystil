...
/// Check if our own app (dystil) is still the active macOS application.
/// When the overlay panel loses focus to another dystil window (Settings,
...
/// If our own app is still active (focus moved to another dystil window
/// like Settings), we just clear without restoring — otherwise Settings/Chat
/// would lose keyboard focus. Also skip restoring if any dystil window
...
pub(crate) fn restore_frontmost_app_if_external_with_app(app: Option<&AppHandle>) {
    ...
    if is_own_app_still_active() {
        info!("Focus moved to another dystil window, clearing saved app without restoring");
        clear_frontmost_app();
        return;
    }
    ...
    if let Some(app) = app {
        ...
        for label in &non_panel_labels {
            if app.get_webview_window(label).is_some() {
                info!(
                    "Dystil '{}' window still open, clearing saved app without restoring",
                    label
                );
                clear_frontmost_app();
                return;
