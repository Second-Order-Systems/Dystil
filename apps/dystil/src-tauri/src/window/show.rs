...
impl ShowRewindWindow {
    ...
    fn show_existing_main(
        ...
        if overlay_mode == "window" {
            ...
            #[cfg(target_os = "windows")]
            {
                ...
                // Without this, re-showing an already-created overlay when another dystil
    ...
    pub fn show(&self, app: &AppHandle) -> tauri::Result<WebviewWindow> {
        ...
        // Closing Home puts macOS into Accessory mode so the app has no Dock or
        // app-switcher presence. Every Home entry point passes through show(),
        // including single-instance relaunch and RunEvent::Reopen.
        #[cfg(target_os = "macos")]
        ...
        if id.label() == RewindWindowId::Onboarding.label() && onboarding_store.is_completed {
        ...
        // === Other windows: standard show path ===
            ...
            if id.label() == RewindWindowId::Home.label() {
                ...
                // Home window may have been hidden from the taskbar/Alt+Tab on
                // close. Restore the native HWND styles before showing it.
                #[cfg(target_os = "windows")]
                if let Err(e) = crate::windows_overlay::restore_window_user_surfaces(&window) {
                    warn!("Failed to restore Home to Windows user surfaces: {e}");
        ...
        let window = match self {
                                    ...
                                    std::thread::spawn(move || {
                                        ...
                                        #[cfg(target_os = "macos")]
                                        {
                                            ...
                                            let _ = app.run_on_main_thread(move || {
                                                ...
                                                // dystil window (Settings, Chat), just clear.
                            ...
                            if !is_focused {
                                ...
                                std::thread::spawn(move || {
                                    ...
                                    #[cfg(target_os = "macos")]
                                    {
                                        ...
                                        let _ = app.run_on_main_thread(move || {
                                            // Conditional restore: if focus moved to another
                                            // dystil window, just clear. Only activate
            ...
            ShowRewindWindow::PermissionRecovery => {
                let builder = self
                    ...
                    // The rounded recovery card includes a full permission row and
                    // needs room to remain a single, non-scrolling view.
    ...
    /// Used when transitioning from Main to another dystil window (e.g. Home/Settings)
        ...
        if id.label() == RewindWindowId::Main.label() {
            #[cfg(target_os = "macos")]
            {
                ...
                run_on_main_thread_safe(app, move || {
                    ...
                    // we're transitioning to another dystil window.
                });
            }
