...
/// Hide the app from the Dock and app switcher while keeping the process alive.
/// Enterprise hidden-UI mode already uses the same activation policy.
#[cfg(target_os = "macos")]
pub fn hide_from_dock(app: &AppHandle) {
    info!("Setting activation policy to Accessory (background mode)");
