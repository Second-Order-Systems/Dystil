...
pub fn install_user_data_dir() {
    ...
    eprintln!(
        "dystil: WebView2 user data dir source={} writable={} path={}{}",
        diagnostics.source,
...
fn choose_user_data_dir() -> Result<(String, PathBuf, String), String> {
    ...
    candidates.push((
        "dystil-data-dir".to_string(),
        crate::dystil_paths::data_dir(),
    ));

    let mut errors = Vec::new();
    for (source, base) in candidates {
        let path = base.join("Dystil").join("WebView2");
...
fn ensure_writable_dir(path: &Path) -> Result<(), String> {
    ...
    let test_path = path.join(format!(".dystil-write-test-{}", std::process::id()));
