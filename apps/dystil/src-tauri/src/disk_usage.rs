...
pub struct CachedDiskUsage {
    pub timestamp: i64,
    pub usage: DiskUsage,
    /// The dystil data directory this entry was computed for. Used to
...
pub async fn disk_usage(
    ...
    let data_dir = dystil_dir.join("data");
    ...
    let current_dir_key = canonical_dir_key(dystil_dir);
        ...
        if let Ok(content) = fs::read_to_string(&cache_file) {
            if content.contains("---") {
                ...
                let one_hour = 60 * 60; // 1 hour cache (reduced from 2 days)
                                        ...
                                        // `dystil_dir` defaults to "" on older cache entries — those
                                        ...
                                        // the same directory.
                let cached_key_normalized = canonical_dir_key(Path::new(&cached.dystil_dir));
                let dir_matches =
                    !cached.dystil_dir.is_empty() && cached_key_normalized == current_dir_key;
                ...
                if !dir_matches {
                    info!(
                        ...
                        cached.dystil_dir, current_dir_key
                    );
                }
            }
        }
    ...
    let total_data_size_bytes = directory_size(dystil_dir)
    ...
    if data_dir.exists() {
        ...
        info!(
            "Video files total: {} bytes, monitors: {:?}",
            total_video_size,
    ...
    if let Ok(entries) = fs::read_dir(dystil_dir) {
    ...
    let pipes_size: u64 = {
        let pipes_dir = dystil_dir.join("pipes");
        if pipes_dir.exists() {
    ...
    let available_space = {
        ...
        let path_obj = Path::new(&dystil_dir);
        let available = sys
            .disks()
            .iter()
