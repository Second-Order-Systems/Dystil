import { generateAppConfigSync, resolveBuildChannel } from "./scripts/app-config.mjs";

// Frontend-only entrypoint guard:
// Next may run without Tauri preflight, so ensure the generated app config
// exists before any browser bundle imports it.
generateAppConfigSync(resolveBuildChannel(process.env));
