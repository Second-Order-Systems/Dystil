...
use crate::screen::browser_utils::create_url_detector;
use crate::screen::monitor::SafeMonitor;

#[cfg(target_os = "macos")]
use crate::screen::monitor::macos_version::use_sck_rs;
...
static SKIP_APPS: Lazy<HashSet<&'static str>> = Lazy::new(|| {
    HashSet::from([
        ...
        "TheBoringNotch",
        "Grammarly Desktop",
        // Dystil's own UI should never be captured
        "dystil",
        "dystil - Development",
        "dystil beta",
    ])
...
impl WindowFilters {
    ...
    /// `AppName::WindowTitle` scope (parsed in `dystil-core::window_pattern`);
    ...
    ///
    /// Delegates to [`crate::a11y::url_filter::is_url_blocked`] so the
    /// vision path and the a11y tree walker agree on what "blocked" means.
    pub fn is_url_blocked(&self, url: &str) -> bool {
        crate::a11y::url_filter::is_url_blocked(url, &self.ignored_urls)
    }
...
/// Skips overlay windows (layer > 0), system apps, and dystil's own UI.
#[cfg(target_os = "macos")]
pub fn find_topmost_pid_on_monitor(
    ...
    for w in cg_windows {
        ...
        // Skip dystil's own UI
        if w.owner_name.to_lowercase().contains("dystil") {
            continue;
        }
...
pub async fn capture_all_visible_windows(
    ...
    for window_data in windows_data {
        ...
        // Safety-net: always exclude dystil's own UI regardless of exact app name variant
        let is_dystil_ui = app_name.to_lowercase().contains("dystil");
        let is_valid = !is_dystil_ui
...
mod tests {
    ...
    mod topmost_tests {
        ...
        #[test]
        fn test_topmost_skips_dystil_ui() {
            let windows = vec![
                make_window(999, 0, 0, 0, 1920, 1080, "dystil", "Main"),
                make_window(100, 0, 0, 0, 1920, 1080, "WezTerm", "π - brain"),
            ];
            assert_eq!(
                find_topmost_pid_on_monitor(&windows, &monitor_1080p()),
        ...
        #[test]
        fn test_topmost_skips_dystil_dev() {
            let windows = vec![
                make_window(999, 0, 0, 0, 1920, 1080, "dystil - Development", "Overlay"),
