...
pub struct TreeWalkerConfig {
    ...
    /// browser tab's URL — see [`crate::a11y::url_filter::is_url_blocked`]).
...
pub enum SkipReason {
    ...
    Incognito,
    /// App is in the hardcoded exclusion list (e.g. dystil, keychain access).
    ExcludedApp,
    ...
    fn walk_focused_window(&self) -> Result<TreeWalkResult> {
        ...
        if let TreeWalkResult::Found(ref snapshot) = result {
            if let Some(ref url) = snapshot.browser_url {
                if crate::a11y::url_filter::is_url_blocked(url, &self.ignored_urls) {
