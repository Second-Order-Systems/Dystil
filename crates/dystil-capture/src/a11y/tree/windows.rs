...
use crate::a11y::events::AccessibilityNode;
use crate::a11y::platform::windows_uia::UiaContext;
...
impl TreeWalkerPlatform for WindowsTreeWalker {
    fn walk_focused_window(&self) -> Result<TreeWalkResult> {
        ...
        if crate::a11y::platform::windows::is_transient_shell_window(hwnd) {
        ...
        let app_name = crate::a11y::platform::windows::get_effective_app_name(hwnd, pid);
        ...
        if self.config.ignore_incognito_windows
            && crate::a11y::incognito::is_title_private(&window_name)
        {
...
fn is_on_screen(bounds: &crate::a11y::events::ElementBounds, window: &WindowRect) -> bool {
...
fn normalize_bounds(
    bounds: &crate::a11y::events::ElementBounds,
    monitor: &MonitorRect,
) -> Option<NodeBounds> {
...
mod tests {
    ...
    fn test_incognito_detection() {
        use crate::a11y::incognito::is_title_private;
