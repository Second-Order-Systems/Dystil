//! macOS incognito detector.
//!
//! Chromium-based browsers expose window properties through AppleScript, but
//! asking for them makes macOS request permission for Dystil to control the
//! browser. We intentionally use the localized title check instead, which
//! keeps private-window exclusion without an Automation prompt.
//!
...
// Browser Automation is deliberately disabled in Dystil. `osascript` queries
// trigger the macOS “Dystil wants to control Google Chrome/Chromium” prompt.
const ALLOW_BROWSER_AUTOMATION: bool = false;
...
impl IncognitoDetector for MacOSIncognitoDetector {
    fn is_incognito(&self, app_name: &str, _process_id: i32, window_title: &str) -> bool {
        ...
        if ALLOW_BROWSER_AUTOMATION && Self::is_chromium_browser(app_name) {
